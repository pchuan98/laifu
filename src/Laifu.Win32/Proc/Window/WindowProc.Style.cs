...
{
    /// <summary>
    /// 该窗口是一个重叠的窗口。与 <see cref="Overlapped"/> 样式相同。
    /// </summary>
    Tiled = 0x00000000,

    /// <summary>
    /// 窗口是一个重叠的窗口。与 <see cref="OverlappedWindow"/> 样式相同。
    /// </summary>
    ...
    /// <summary>
    /// 窗口有一个最大化按钮。 不能与 <see cref="WindowStyleEx.ContextHelp"/> 样式组合使用。 还必须指定 <see cref="SysMenu"/> 样式。
    /// </summary>
    ...
    /// <summary>
    /// 窗口是一个重叠的窗口。与 <see cref="Tiled"/> 样式相同。
    /// </summary>
    TiledWindow = (Overlapped | Caption | SysMenu | ThickFrame | MinimizeBox | MaximizeBox),

    /// <summary>
    /// 窗口是一个重叠的窗口。与 <see cref="Overlapped"/> 样式相同。
    /// </summary>
...
public partial class WindowProc
{
    ...
    /// </summary>
    public IEnumerable<string> Styles => Enum.GetNames(typeof(WindowStyle)).Where(name => IsStyle(Enum.Parse<WindowStyle>(name)));
    ...
    public void AddStyle(WindowStyle style) =>
        NativeHelper.SetWindowLong(Hwnd, WINDOW_LONG_PTR_INDEX.GWL_STYLE, LStyles | (long)style);
    ...
    /// <param name="style"></param>
    /// <returns></returns>
    public bool IsStyle(WindowStyle style) => ((long)style & LStyles) == (long)style;

    /// <summary>
    /// 窗口的扩展样式（WS_EX_开头的常量），提供额外的外观和行为选项。
    /// </summary>
    public IEnumerable<string> ExtendedStyles => Enum.GetNames(typeof(WindowStyleEx)).Where(name => IsExtendedStyle(Enum.Parse<WindowStyleEx>(name)));

    /// <summary>
    /// 
    /// </summary>
    /// <param name="style"></param>
    public void AddExtendStyle(WindowStyleEx style) =>
        NativeHelper.SetWindowLong(Hwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, LExtendedStyles | (long)style);

    /// <summary>
    /// 
    /// </summary>
    /// <param name="style"></param>
    public void RemoveExtendStyle(WindowStyleEx style) =>
        NativeHelper.SetWindowLong(Hwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, LExtendedStyles & (~(long)style));
    ...
    /// <param name="style"></param>
    /// <returns></returns>
    public bool IsExtendedStyle(WindowStyleEx style) => ((long)style & LExtendedStyles) == (long)style;

    /// <summary>
    /// 
    /// </summary>
    public long LExtendedStyles => NativeHelper.GetWindowLong(Hwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE);

    public double Opacity
    {
        set
        {
            AddExtendStyle(WindowStyleEx.Layered);

            if (!IsExtendedStyle(WindowStyleEx.Layered))
            {
                Log.Warning(
                    "Set Window Opacity must append the LayeredExStyle. But this window cant add this Ex style. ");
                return;
            }

            //if(IsExtendedStyle(WindowStyleEx.Transparent))

            Log.Information("test with lib");

            if (PInvoke.SetLayeredWindowAttributes(Hwnd, new COLORREF(255), (byte)(value * 255),
                    LAYERED_WINDOW_ATTRIBUTES_FLAGS.LWA_ALPHA) != 0) return;

            var code = ErrorHelper.ErrorCode;
            var str = ErrorHelper.ErrorCode2String((uint)code);

            throw new Exception($"Set Window Opacity Error. Message is '{str}'");
        }
    }
